...
def fetch_player_matches_ids(riot_api: RiotAPI, player_puuid: str, start_time: int|None = None):
    url = 'https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/' + player_puuid + '/ids?queue=420&type=ranked'
    if start_time:
...
def clean_match_data(response):
    filtered_rows = [
        'puuid', 'metadata.matchId', 'kills', 'assists', 'deaths',
        'championName', 'champLevel', 'champExperience',
        'goldEarned', 'goldSpent', 'teamPosition', 'teamId',
        'totalDamageDealtToChampions', 'totalMinionsKilled',
        'gameEndedInSurrender', 'gameEndedInEarlySurrender', 'visionScore', 'win'
    ]

    participants_infos = pd.json_normalize(response, record_path=[['info', 'participants']], meta=[['metadata', 'matchId']])
    ...
    match_infos = match_infos[
        ['metadata.matchId', 'info.gameVersion', 'info.gameDuration', 'team_win', 'info.gameCreation', 'info.gameStartTimestamp', 'info.gameEndTimestamp']]

    match_infos = match_infos.rename(columns={'metadata.matchId': 'id', 'info.gameVersion': 'game_version', 'info.gameDuration': 'duration',
        'info.gameCreation': 'created_at', 'info.gameStartTimestamp': 'started_at', 'info.gameEndTimestamp': 'ended_at'
    })

    return { 'participants_infos': participants_infos, 'match_infos': match_infos, 'players_infos': players_infos }
...
def match_to_tuple(match_infos_df):
    ...
    return tuple(
        ...
        for col in ['id', 'game_version', 'duration', 'team_win', 'created_at', 'started_at', 'ended_at']
    )
